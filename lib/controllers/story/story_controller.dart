...
class AppStoryController extends GetxController {
  ...
  void showHideEmoticons(bool show) {
    showEmoticons.value = show;
  }

  void replyTextChanged(String text) {
  ...
  void deleteStory(VoidCallback callback) {
    StoryApi.deleteStory(
        id: currentStoryMediaModel.value!.id, callback: callback);
  }

  void setCurrentStoryMedia(StoryMediaModel storyMedia) {
  ...
  void sendTextMessage(String message, StoryModel story) {
  ...
  void sendReactionMessage(String emoji, StoryModel story) {
    _chatDetailController.getChatRoomWithUser(
        userId: currentStoryMediaModel.value!.userId,
        callback: (room) {
          FocusScope.of(Get.context!).requestFocus(FocusNode());
  ...
  void sendStoryAsMessage(int userId, StoryModel story) {
    _chatDetailController.getChatRoomWithUser(
        userId: userId,
        callback: (room) {
          FocusScope.of(Get.context!).requestFocus(FocusNode());
