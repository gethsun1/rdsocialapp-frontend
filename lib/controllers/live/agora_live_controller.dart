...
class AgoraLiveController extends GetxController {
  ...
  Future<void> checkFeasibilityToLive(
  ...
  Future<void> joinAsAudience({required LiveModel live}) async {
  ...
  Future<void> _joinLive({required LiveModel live}) async {
    ...
    try {
      ...
      errorMessage = 'Live initialization failed. Please verify Agora keys.';
      startLiveStreaming.value = LiveStreamingStatus.failed;
      debugPrint('[AgoraLiveController] Live initialization failed: $e');
  ...
  Future<void> _initAgoraRtcEngine() async {
    try {
      ...
      debugPrint('[AgoraLiveController] Agora engine init failed: $e');
      debugPrint(st.toString());
      engine = null;
      rethrow;
    }
    ...
    engine?.registerEventHandler(
      RtcEngineEventHandler(
          onPermissionError: (permission) {
            print('checking live  ${permission.name}');
          },
          onJoinChannelSuccess: (RtcConnection connection, int elapsed) async {
            print(
                "checking live local user ${connection.localUid} joined , on channel ${connection.channelId}");
            update();
          },
          onLeaveChannel: (RtcConnection connection, RtcStats status) {
            print('checking live  onLeaveChannel');
          },
          onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) {
          ...
          onConnectionStateChanged: (RtcConnection connection,
              ...
              ConnectionChangedReasonType reason) async {
            print(
                'checking live onConnectionStateChanged $connection state name =  ${state.name} reason = ${reason.name}');
          ...
          onFirstRemoteVideoFrame: (RtcConnection connection, int remoteUid,
            ...
            print('checking live onFirstRemoteVideoFrame');
          },
          onRemoteVideoStateChanged: (RtcConnection connection,
              int remoteUid,
              RemoteVideoState state,
              RemoteVideoStateReason reason,
              int elapsed) async {
            print('checking live onRemoteVideoStateChanged');
          ...
          onLocalVideoStateChanged: (VideoSourceType source,
            ...
            print('checking live onLocalVideoStateChanged state=$state');
            print('checking live onLocalVideoStateChanged source=$source');
            print('checking live onLocalVideoStateChanged reason=$reason');
          ...
          onFirstLocalVideoFrame:
            ...
            print('checking live onFirstLocalVideoFrame');
          },
          onFirstLocalVideoFramePublished:
              (RtcConnection connection, int elapsed) {
            print('checking live onFirstLocalVideoFramePublished');
          }),
    );
  }
  ...
  void sendGiftMessage(String giftImage, int coins) {
    String localMessageId = randomId();
  ...
  void sendGift({required GiftModel gift, LiveCallHostUser? host}) {
  ...
  void buyPackage(PackageModel package) {
  ...
  void onToggleMuteVideo() {
    ...
    print('mutedVideo ${mutedVideo.value}');
  }
  ...
  void invitedForLiveBattle(LiveModel live) {
  ...
  void startLive({required BattleDetail battleDetail}) {
    Timer(const Duration(seconds: 2), () {
  ...
  void declineInvite(BattleDetail battleDetail) {
    getIt<SocketManager>().emit(SocketConstants.replyInvitationInLive, {
      'battleId': battleDetail.id,
      'status': 3,
    });
  ...
  void liveBattleEnded({required int liveId, required int battleId}) {
    clearGiftData();
  ...
  void onNewUserJoined(UserModel user, int liveId, int totalUsers) {
  ...
  void onLiveEndMessageReceived(int liveId) {
    engine?.leaveChannel();
  ...
  void onNewMessageReceived(ChatMessageModel message) {
  ...
  void liveCreatedConfirmation(dynamic data) {
  ...
  void loadGiftsReceived({required int liveId, int? battleId}) {
    if (canLoadMoreGifts) {
  ...
  void banUser(UserModel user, int? time) {
    getIt<SocketManager>().emit(
  ...
  void makeModerator(UserModel user) {
    getIt<SocketManager>().emit(
        SocketConstants.actionOnUserInLive,
        ({
          'actionUserId': user.id,
  ...
  void removeAsModerator(UserModel user) {
    getIt<SocketManager>().emit(
        SocketConstants.actionOnUserInLive,
        ({
          'actionUserId': user.id,
  ...
  void loadMoreLiveViewers(VoidCallback callback) {
  ...
  void refreshLiveViewers() {
    liveViewers.clear();
    liveViewersDataWrapper = DataWrapper();
    getLiveViewers(() {});
  }

  void getLiveViewers(VoidCallback callback) {
    liveViewers.clear();
  ...
  void loadMoreModerators(VoidCallback callback) {
  ...
  void getModerator(VoidCallback callback) {
    LiveStreamingApi.getLiveViewers(
        liveId: live.value!.id,
  ...
  void loadMoreBannedUsers(VoidCallback callback) {
  ...
  void refreshBannedViewers() {
    bannedUsers.clear();
    bannedUsersDataWrapper = DataWrapper();
    getBannedUser(() {});
  }

  void getBannedUser(VoidCallback callback) {
    LiveStreamingApi.getLiveViewers(
        liveId: live.value!.id,
        isBanned: true,
        resultCallback: (result, metaData) {
