...
class ChatDetailController extends GetxController {
  ...
  void deleteChat(int roomId) {
    messages.clear();
    ChatApi.deleteChatRoomMessages(roomId);
    update();
  }

  void getUpdatedChatRoomDetail(
  ...
  Future<void> loadChat(ChatRoomModel chatRoom, VoidCallback completion) async {
  ...
  Future<void> loadWallpaper(int roomId) async {
    wallpaper.value = await SharedPrefs().getWallpaper(roomId: roomId);
  }

  void createChatRoom(int userId, Function(int) callback) {
  ...
  bool isSelected(ChatMessageModel message) {
    return selectedMessages.contains(message);
  }

  void starMessage(ChatMessageModel message) {
    message.isStar = 1;
  ...
  void unStarMessage(ChatMessageModel message) {
    message.isStar = 0;
    selectedMessages.refresh();
    getIt<DBManager>().starUnStarMessage(
        roomId: message.roomId,
  ...
  void setToActionMode({required ChatMessageActionMode mode}) {
  ...
  void setReplyMessage({required ChatMessageModel? message}) {
    if (message == null) {
...
// }
  ...
  void sendMessageAsRead(ChatMessageModel message) {
  ...
  Future<void> deleteMessage({required int deleteScope}) async {
...
//*************** updates from socket *******************//
  ...
  Future<void> messageUpdateReceived(Map<String, dynamic> updatedData) async {
  ...
  void updatedChatGroupAccessStatus(
