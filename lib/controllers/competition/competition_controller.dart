...
class CompetitionController extends GetxController {
  ...
  Future<void> getCompetitions(VoidCallback callback) async {
  ...
  void setCompetition(CompetitionModel modal) {
    competition.value = modal;
    update();
  }

  void loadCompetitionDetail({required int id}) {
  ...
  Future<void> viewMySubmission(CompetitionModel competition) async {
  ...
  Future<void> submitMedia(CompetitionModel competition) async {
    if (competition.competitionMediaType == 1) {
